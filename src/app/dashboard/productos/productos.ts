...
export class Productos implements OnInit {
  ...
  guardarProducto() {
    ...
    if (!this.productoForm.nombre || categoriaNumerica === 0) {
      Swal.fire('Error', 'El nombre y la categoría son obligatorios.', 'error');
      return;
    }

    // PVP ya no se edita en el modal: al crear va 0; al editar se conserva el actual
    const precioNumerico = Number(this.productoForm.precioUnitario) || 0;
    ...
    if (this.isEditing && this.currentProductId) {
      this.http.put(
        `${this.apiUrl}/negocios/${this.negocioId}/productos/${this.currentProductId}`,
        formData,
        { headers }
      ).subscribe({
      ...
      this.http.post(
        `${this.apiUrl}/negocios/${this.negocioId}/productos`,
        formData,
        { headers }
      ).subscribe({
  ...
  eliminarProducto(id: number) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const headers = this.getAuthHeaders();
        this.http.delete(
          `${this.apiUrl}/negocios/${this.negocioId}/productos/${id}`,
          { headers }
        ).subscribe({
          next: () => {
